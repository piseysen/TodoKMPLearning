...
class HomeViewModel(private val realmMongoDB: RealmMongoDB): ScreenModel {
    ...
    init{
        ...
        screenModelScope.launch(Dispatchers.Main) {
            delay(500)
            realmMongoDB.readActiveTasks().collectLatest {
        ...
        screenModelScope.launch(Dispatchers.Main) {
            delay(500)
            realmMongoDB.readCompletedTasks().collectLatest {
    ...
    private fun setCompleted(task: ToDoTask, completed: Boolean) {
        screenModelScope.launch(Dispatchers.IO) {
            realmMongoDB.setCompleted(task, completed)
        }
    }

    private fun setFavorite(task: ToDoTask, isFavorite: Boolean) {
        screenModelScope.launch(Dispatchers.IO) {
            realmMongoDB.setFavorite(task, isFavorite)
        }
    }

    private fun deleteTask(task: ToDoTask) {
        screenModelScope.launch(Dispatchers.IO) {
            realmMongoDB.deleteTask(task)
        }
    }
